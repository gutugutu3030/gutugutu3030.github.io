$(function(){
    ...
    $("#calcTime").submit(function(){
        ...
        if(shootingTime.toString()=="Invalid Date"
        ...
        try {
            ...
            $("#time").text("時間："+ (date.getMonth()+1)+"/"+date.getDate()+" "
...
function getDif(target, shooting){
    var dateDif = (Math.floor(shooting.getTime() / dateMillis) - Math.floor(target.getTime() / dateMillis));
    return dateDif * dateMillis / 365 + shooting.getTime() % dateMillis;
}

function getDate(shootingPer, targetPer, targetDate){
    return new Date(targetDate.getTime() + (shootingPer - targetPer) * dateMillis);
}